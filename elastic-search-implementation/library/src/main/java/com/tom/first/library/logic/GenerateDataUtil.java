...
public class GenerateDataUtil {
	...
	protected String generatePassword() {
		return generate(() -> faker.get().credentials().password(8, 12));
	}
	
	protected String generateEmail() {
		return generate(() -> faker.get().internet().emailAddress());
	}
	
	protected String generateUsername() {
		return generate(() -> faker.get().credentials().username());	
	}
	
	
	protected String generateDescription(){
		return generate(() -> faker.get().restaurant().description());
	}
	
	protected String generateSubject() {
		return generate(() -> faker.get().internet().emailSubject());
	}
	...
	protected LocalDate getRandomDate(LocalDate minDate, LocalDate maxDate) {
		...
		long randomEpochDay = loc.nextLong(minDay, maxDay + 1);
